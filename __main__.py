...
arg ...: arguments passed to program in sys.argv[1:]
    ...
    exec(codeobj, {'__name__':'__main__', '__file__':codeobj.co_filename})
elif options.get('-c') is not None:
    exec(options.get('-c'), main.__dict__)
elif sys.argv[0]:
    if sys.argv[0].endswith(('.zip', 'egg')):
        import zipimport
    ...
    __import__('code').interact(banner='Python %s on %s\n%s' % (sys.version, sys.platform, cprt))
